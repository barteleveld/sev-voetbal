...
const SEARCH_QUERIES = [
  { query: "investering", pages: 3 },
  { query: '"club van 50"', pages: 2 },
  { query: '"mede mogelijk gemaakt"', pages: 2 },
  { query: "bedankt", pages: 2 },
  { query: "bijdrage", pages: 2 },
  { query: "dankzij", pages: 2 }
];
...
function parseItem(xml, index) {
  ...
  const rawTitle = cleanText(tag(xml, "title"));
  ...
  return {
    ...
    searchText: `${rawTitle} ${cleanText(description)}`
  };
}
...
function searchFeedUrl(query, page = 1) {
  const params = new URLSearchParams({ s: query, feed: "rss2" });
  if (page > 1) params.set("paged", String(page));
  return `https://www.sev-voetbal.nl/?${params}`;
}

function mentionsInvestment(item) {
  return /\bclub\s+van\s+50\b/i.test(item.searchText)
    || /(?:^|\W)(?:[Dd]e\s+)?[Ii]nVESTEring(?:\W|$)/.test(item.searchText)
    || /\bde\s+investering\b/i.test(item.searchText);
}

function publicItem({ searchText, ...item }) {
  return item;
}
...
export async function getInvestmentNews(limit = 30) {
  ...
  const sources = SEARCH_QUERIES.flatMap(({ query, pages }) =>
    Array.from({ length: pages }, (_, index) => ({
      url: searchFeedUrl(query, index + 1),
      label: `${query}-${index + 1}`
    }))
  );
  const results = await Promise.allSettled(sources.map(({ url, label }) => fetchFeed(url, label)));
  const items = results
    .flatMap((result) => result.status === "fulfilled" ? result.value : [])
    .filter(mentionsInvestment)
